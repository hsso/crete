...
c     fac      -- maxgau-1/maxsig: divide maxgau over required sigma's
c     gauvals  -- values of gaussian at step i
c     gaunorm     -- normalization factor
...
c     Follow profile out to 4 sigma (exp(-4)**2 = 1e-7)
      ...
      INTEGER maxgau,maxsig,ival
      DOUBLE PRECISION fac
      PARAMETER (maxgau = 101)
      PARAMETER (maxsig = 4)
      PARAMETER (fac = (maxgau-1)/maxsig)
      INTEGER igau
      DOUBLE PRECISION v,sigma,gauss
      DOUBLE PRECISION gauvals(maxgau),val
      SAVE gauvals
...
c     If the gaussian profile is not yet calculated... do it
      ...
      if (gauvals(1).lt.1.d-6) then
        do igau = 1,maxgau
          val = (dble(igau)-1.d0)/fac
          gauvals(igau) = dexp(-(val**2.d0))
        enddo
      endif
...
c     Explicitely take Wien approximation for h*nu>>k*T:
        if (hplanck*nu(iline).gt.100.d0*kboltz*t) then
          planck=2.d0*hplanck*((nu(iline)/clight)**2.d0)*nu(iline)
     $      *dexp(-hplanck*nu(iline)/kboltz/t)
        else
          planck=2.d0*hplanck*((nu(iline)/clight)**2.d0)*nu(iline)
     $      /(dexp(hplanck*nu(iline)/kboltz/t)-1.d0)
        endif
      endif
